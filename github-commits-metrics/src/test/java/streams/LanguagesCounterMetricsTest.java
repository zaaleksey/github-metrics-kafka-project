...
class LanguagesCounterMetricsTest {
  ...
  void setUp() {
    ...
    MetricsStream metricsStream = new LanguagesCounterMetrics(properties, INPUT_TOPIC, OUTPUT_TOPIC);
  ...
  void languagesAreCounted() throws JsonProcessingException {
    ...
    assertEquals("java: 1", outputTopic.readValue());
    assertEquals("python: 1", outputTopic.readValue());
    assertEquals("java: 2", outputTopic.readValue());
  ...
  void closeKafkaStream() {
    MetricsStream metrics =
        new LanguagesCounterMetrics(new Properties(), INPUT_TOPIC, OUTPUT_TOPIC);
